...
class Simulation:
    ...
    def get_wait_time_mean_from_input(self, default_value):
        while True:
            input_wait_time_mean = input(
                "What is the mean wait time in ms of each sender? Please provide a positive float value. If you do not provide a value, the default value of {default_value} will be used.\n".format(default_value=default_value))
            if input_wait_time_mean.strip() == "":
                return default_value
            try:
                wait_time_mean = float(input_wait_time_mean)
                # Negative number
                if wait_time_mean > 0:
    ...
    def get_wait_time_std_from_input(self, default_value):
        while True:
            input_wait_time_std = input(
                "What is the standard deviation of the wait time in ms of each sender? Please provide a positive float value. If you do not provide a value, the default value of {default_value} will be used.\n".format(default_value=default_value))
            if input_wait_time_std.strip() == "":
                return default_value
            try:
                wait_time_std = float(input_wait_time_std)
                # Negative number
                if wait_time_std > 0:
    ...
    def main(self):
        ...
        producer.generate_messages(max_message_length, num_messages)
        ...
        for i in range(num_senders):
            for j in range(num_messages_per_sender):
                ...
                senders[i].add_messages_to_queue(message)
